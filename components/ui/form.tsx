...
type FormFieldProps<TFieldValues extends FieldValues, TName extends FieldPath<TFieldValues>> = Omit<
	...
	'render'
> & {
	children: React.ReactNode | ((field: FormFieldContextValue) => React.ReactNode);
	error?: string | null;
};
				...
				return (
					<FormFieldContext.Provider value={fieldValue}>
						<div data-slot="form-field">
							{typeof children === 'function' ? children(fieldValue) : children}
						</div>
...
function FormControl({ className, children, ...props }: FormControlProps) {
	...
	const handleChange = (value: unknown) => {
		const normalizedValue =
			typeof value === 'object' && value !== null && 'target' in value
				? (() => {
						const target = (
							value as { target?: { value?: unknown; checked?: boolean } }
						).target;
	...
	return (
		<div data-slot="form-control" className={cn('w-full', className)} {...props}>
			{React.cloneElement(children, {
				id: childProps.id ?? field.id,
				name: childProps.name ?? field.name,
				value: field.value !== undefined ? field.value : childProps.value,
				onChange: handleChange,
				onValueChange: childProps.onValueChange ? handleChange : undefined,
				onBlur: handleBlur,
