...
const ALCHEMY_SEPOLIA_URL = process.env.NEXT_PUBLIC_RPC_URL;
...
// WETH合约地址（sepolia测试网）
const addressWETH = process.env.NEXT_PUBLIC_SOPOLIA_ADDRESS_WETH;
// 声明WETH合约
...
const main = async () => {
	...
	if (
		...
		for (let i = 0; i < numWallet; i++) {
			...
			console.log(
				`第 ${i + 1} 个钱包 ${wallets[i].address} WETH 归集开始`
			);
		}
		await tx.wait();
